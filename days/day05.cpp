...
enum state { READING_RANGES, READING_INGREDIENTS };
...
public:
    ...
    static std::optional<Range> merge(const Range& r1, const Range& r2) {
        ...
        return Range(std::min(r1.lower, r2.lower),
...
long day05_part1() {
    ...
    for (std::string line : data) {
        ...
        if (state == READING_RANGES) {
            ...
            // We're ASSUMING here that `range_data` only decomposes to two
            // elements.
            ranges.push_back(
                Range{std::stol(range_data[0]), std::stol(range_data[1])});
