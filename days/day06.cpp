...
long day06_part1() {
    ...
    for (size_t col = 0; col < operations.size(); ++col) {
        ...
        // NOTE: Assumes, there's more than one operand per line, which seems to
        // be the case.
...
std::string extract_column(const std::vector<std::string>& data, size_t col) {
    std::string result;
...
std::vector<long> extract_operands(std::vector<std::string>& data, size_t col,
                                   int offset) {
...
long day06_part2() {
    ...
    data.pop_back(); // important, so we can do loops on data w/o having to
                     // worry abt the last row
    ...
    for (size_t i = 0; i + 1 < operation_locations.size(); ++i) {
        operation_offsets.push_back(operation_locations[i + 1] -
                                    operation_locations[i] - 2);
    }

    // Last item
    operation_offsets.push_back(last_row.size() - operation_locations.back() -
                                1);

    for (size_t current_op_index = 0;
         current_op_index < operation_locations.size(); ++current_op_index) {
        // Operands are numbers to be operated
        std::vector<long> operands =
            extract_operands(data, operation_locations[current_op_index],
                             operation_offsets[current_op_index]);

        // Should check if operands are not empty but don't have to
        // realistically
        long sub_total = operands[0];
        for (size_t i = 1; i < operands.size(); ++i) {
            sub_total = (operations[current_op_index] == ADD)
                            ? sub_total + operands[i]
                            : sub_total * operands[i];
        }
